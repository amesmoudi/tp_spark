...
.add("flagForWcs" ,LongType ,true)
    ...
    val spark = SparkSession.builder.appName("Question quatre").getOrCreate()
    val collisions =spark
        .read
        .option("inferSchema","true").option("header","true").schema(schema_source)
        .csv(inputDir)
import spark.implicits._
    val donneesDF = collisions.select($"objectId",$"ra",$"decl").where($"scienceCcdExposureId"===453349688988l).filter($"objectId".isNotNull)
    donneesDF.write
        .option("header",true)
        .option("sep",";")
        .csv(outputDir)
