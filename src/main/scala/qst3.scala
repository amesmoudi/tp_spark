...
.add("flagForWcs" ,LongType ,true)
    ...
    val schema_file = args(2)
    val spark = SparkSession.builder.appName("Question trois").getOrCreate()
    val collisions =spark
        .read
        .option("inferSchema","true").option("header","true").schema(schema_source)
        .csv(inputDir)
import spark.implicits._
    val donneesDF = collisions.select($"sourceId",$"objectId").where($"ra".between(358.0,359.0) and $"decl".between(2.7,3.0)).filter($"objectId".isNotNull)
    donneesDF.write
        .option("header",true)
        .option("sep",";")
        .csv(outputDir)
