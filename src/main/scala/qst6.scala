...
.add("flagForWcs" ,LongType ,true)
    ...
    val spark = SparkSession.builder.appName("Question six").getOrCreate()
    val collisions =spark
        .read
        .option("inferSchema","true").option("header","true").schema(schema_source)
        .csv(inputDir)
import spark.implicits._
   val donneesDF = collisions.groupBy($"objectId").agg(count($"sourceId").as("nb_observation")).filter($"objectId".isNotNull).select($"objectId",$"nb_observation")
    donneesDF.write
        .option("header",true)
        .option("sep",";")
        .csv(outputDir)
