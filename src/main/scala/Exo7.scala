...
object Exo7 {
  def main(args: Array[String]) {
    ...
    val customSchemaObjets = (new StructType)
      .add("objectId", IntegerType, false)
      ...
      .add("uRaOffset_SG FloatType", FloatType, true)
    ...
    val dataObservations = spark
        .read
        .schema(customSchemaObservations)   
        .csv(inputObservations)

    val dataObjets = spark
        .read
        .schema(customSchemaObjets)   
        .csv(inputObjets)
    ...
    val donneesDF = dataObservations.select($"sourceId",$"objectId")
        ...
        .join(dataObjets, Seq("objectId"))

    // val res = donneesDF.join(dataObjets,donneesDF("objectId") ===  dataObjets("objectId"), "inner") --- ERROR DEPRECATED COLUMNS
    // val res = donneesDF
