...
val schema_source = new StructType()
...
.add("flagForAssociation" ,LongType ,true)
.add("flagForDetection" ,LongType ,true)
.add("flagForWcs" ,LongType ,true)
    val inputDir = args(0)
    val outputDir = args(1)
    val spark = SparkSession.builder.appName("Question cinq").getOrCreate()
    val collisions =spark
        .read
        .option("inferSchema","true").option("header","true").schema(schema_source)
        .csv(inputDir)
import spark.implicits._
    val donneesDF = collisions.where('ra.between(358.0,359.0) and 'decl.between(2.7,3.0))
            .filter($"objectId".isNotNull).groupBy($"objectId").agg(count("*").as("nb_observation")).select($"objectId",$"nb_observation")
    donneesDF.write
        .option("header",true)
        .option("sep",";")
        .csv(outputDir)
